...
def _rec(**kw):
    base = dict(
        source="usgs_feed", kind="quake", payload={"a": 1},
...
def test_envelope_and_append(tmp_path):
    ...
    path = tmp_path / "bronze/usgs_feed/quake/dt=2026-08-11/part-06.jsonl"
...
def test_partition_by_hour_and_source(tmp_path):
    ...
    sink.write([_rec(), _rec(fetched_at=ts_next_hour), _rec(source="bbc", kind="news")])
