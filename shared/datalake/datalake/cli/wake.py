"""uv run datalake-wake — AISStream 스트림 구독 수집.

배치 운영: --duration N 초 동안 구독 후 종료 (Temporal 등에서 겹치지 않게
스케줄). --duration 0(기본)은 Ctrl-C까지 상시 구독. DATALAKE_AIS_KEY 필요
...
async def _run(args) -> int:
    client = wake.build(preset=args.preset)
    if client is None:
        logging.basicConfig(level=logging.INFO)
        log.error("wake 비활성: DATALAKE_AIS_KEY 미설정 (전용 키 필요)")
...
def main(argv: list[str] | None = None) -> int:
    parser = _common.base_parser("datalake-wake", __doc__)
    parser.add_argument("--duration", type=float, default=0.0,
                        help="구독 유지 시간(초). 0 = Ctrl-C까지 (기본)")
    parser.add_argument("--preset", default=None,
                        choices=sorted(wake.PRESETS),
                        help="관심 해역 (기본: env DATALAKE_WAKE_PRESET=kr)")
