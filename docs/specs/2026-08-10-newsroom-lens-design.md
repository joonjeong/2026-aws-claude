...
## 11. 보너스 카드 A–D (2026-08-10 추가 구현)

랩 문서(Capstone 6)의 보너스 카드 4종. E(자유 기능)는 별도 아이디어 대기.

### A. 5번째 소스 — Al Jazeera
- `config.SOURCES`에 `aljazeera` 추가 (공식 `aljazeera.com/xml/rss/all.xml`,
  2026-08-10 HTTP 200 / RSS 2.0 확인). 수집기·스토어·API는 원래 SOURCES 순회라 무변경.
- 렌즈 시스템 프롬프트를 `build_system_prompt()`로 동적 생성 — 매체 목록,
  frames/tones/sources 스키마가 SOURCES에서 자동 확장. 이후 소스 추가는 config 1항목.
- 그리드/프레임/상태바 CSS를 고정 4열 → `repeat(auto-fit, minmax(…, 1fr))`.
  검증: 5열 균등 트랙, 잉여 트랙 0px 붕괴.

### B. 키워드 핀
- 툴바 입력 + 칩 목록, `localStorage["news-pins"]` 영속(JSON 배열, 대소문자 무시 중복 제거).
- 하이라이트: 핀들을 escape 후 단일 정규식 `(k1|k2)` gi 분할 → `<mark>` 렌더.
- "핀만 보기" 토글(`news-pins-filter`): 제목+요약에 핀 포함 기사만 그리드에 표시,
  매체별 0건이면 안내 카드.

### C. 논조 온도 (렌즈 확장)
- 프롬프트 스키마에 `tones{매체: -2..+2 정수}` 추가. 미보도 매체는 키 제외 규칙.
- 백엔드는 클러스터 패스스루라 코드 무변경(프롬프트만). 프론트 `ToneBar`:
  파랑(-2 비판)↔회색(0)↔빨강(+2 옹호) 그라데이션 트랙 + 위치 도트 + 라벨.
- 모델이 tones를 빠뜨려도 렌더 생략만 되는 방어적 `typeof === "number"` 가드.

### D. 브리핑 복사
- 렌즈 헤더에 "브리핑 복사" 버튼 → Slack 친화 플레인텍스트
  (*볼드* 마커, • 프레임(논조 n): …, 근거 제목 — URL). clipboard API + textarea 폴백,
  2초 "복사됨 ✓" 피드백.

검증(2026-08-10, 로컬 :8010 + Playwright): 5소스 각 15건 수집, 5열 그리드,
핀 하이라이트 9건·필터 75→4건·새로고침 영속, 렌즈 목킹으로 톤 바 4개
(-1/-2/0/+2 위치 25/0/50/100%)·미보도 프레임 제외·브리핑 텍스트 계약 확인, JS 에러 0.
